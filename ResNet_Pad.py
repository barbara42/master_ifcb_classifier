...
# Data augmentation - padding all the images to the max image dimensions
# TODO: what happens when the image is larger than the max width and height? 
height = 224
width = 224
maxpad_data_transforms = {
...
train_dataset = torchvision.datasets.ImageFolder(f"{data_dir}/train", maxpad_data_transforms["train"])
val_dataset = torchvision.datasets.ImageFolder(f"{data_dir}/test", maxpad_data_transforms["val"])
...
# exp_lr_scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer_ft, mode='min', factor=0.1, patience=3, min_lr=1e-6)
