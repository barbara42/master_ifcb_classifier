...
DEST_ROOT = "/nobackup/users/birdy/resnet50-stage1-output-2"
dataset_name = "UH"
...
optimizers = ["adamw"] # just need to run adamw this time 
...
for opt in optimizers:
  for sched in schedulers:
    for crit, criterion in zip(criterion_labels, criterions):
      ...
      model_name = f"resnet50-{dataset_name}-{opt}-{sched}-{crit}"
      ...
      print(results_df)
      # save dataframe 
      results_df.to_csv(f"{DEST_ROOT}/resnet50_{dataset_name}_stage1_results.csv", index=False)
      print("")
