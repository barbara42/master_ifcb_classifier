...
DEST_ROOT =f"/nobackup/users/birdy/resnetAll-{dataset_name}-stage3-output2"
...
#     train_dataset = helper.WHOIDataset(data_dir, csv_file, "train", transform=helper.data_transforms[t])
#     val_dataset = helper.WHOIDataset(data_dir, csv_file, "val", transform=helper.data_transforms[v])
...
#     results_df.to_csv(f"{DEST_ROOT}/{dataset_name}_stage3_results.csv", index=False)
#     print(results_df)
#     print("")
...
for t, v in zip(train_aug_key, val_aug_key):
    ...
    # Load the pretrained ResNet18 model.
    model_name = f"resnet50-{dataset_name}-{t}"
...
for t, v in zip(train_aug_key, val_aug_key):
    ...
    # Load the pretrained ResNet18 model.
    model_name = f"resnet152-{dataset_name}-{t}"
