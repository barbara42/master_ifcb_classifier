...
data_dir = "/nobackup/projects/public/WHOI-Plankton/2014"
base_model = "resnet18"
DEST_ROOT = f"/nobackup/users/birdy/{base_model}-stage1-{dataset_name}-output"
...
criterions = [cross_entropy_loss]
criterion_labels = ["cross_entropy_loss"]
# Set Up Optimizers:
...
for opt in optimizers:
  for sched in schedulers:
    for crit, criterion in zip(criterion_labels, criterions):
      ...
      model_name = f"{base_model}-{dataset_name}-{opt}-{sched}-{crit}"
      ...
      print(results_df)
      # save dataframe 
      results_df.to_csv(f"{DEST_ROOT}/{base_model}_{dataset_name}_stage1_results.csv", index=False)
      print("")
