...
train_dataset = torchvision.datasets.ImageFolder(f"{data_dir}/train", data_transforms["train_augmix"])
...
# calculate weights for weightedSamples with the WHOLE dataset, THEN assign to samples weight
#calculate the weight
# labels = image_dataset.targets
labels = train_dataset.targets
class_sample_count = np.array(
    [len(np.where(labels == t)[0]) for t in np.unique(labels)])
class_sample_count_clamped = np.array([min(1000, max(100, count)) for count in class_sample_count])
class_weights = 1. / class_sample_count_clamped
# class_weights = 1. / class_sample_count
# create vector of weights for each sample in the training set
y_train_indices = train_dataset.indices
y_train = [train_dataset.targets[i] for i in y_train_indices]
training_sample_weights = np.array([class_weights[t] for t in y_train])
train_sampler = torch.utils.data.sampler.WeightedRandomSampler(training_sample_weights, len(training_sample_weights), replacement=True)


BATCH_SIZE = 512
dataloaders  = {'train': torch.utils.data.DataLoader(train_dataset, batch_size=BATCH_SIZE, num_workers=4, sampler=train_sampler, drop_last=True),
                'val': torch.utils.data.DataLoader(val_dataset, batch_size=BATCH_SIZE, shuffle=True, num_workers=4, drop_last=True)}
...
##########################################################################
############## Weighted Loss Function - Classes of Interest           ########################
##########################################################################
torch.cuda.empty_cache()
...
# add class_weights to cross entropy loss 
# list of classes we are the most interested in 
classes_of_interest = [47, 73, 85, 16, 58, 113, 36, 155, 81, 36] 
# classes of interest get weight of 1.0, everything else gets weight 0.6
class_weights = torch.full(len(class_names), 0.6)
...
##########################################################################
##############  Weighted Loss Function - Rebalanced           ########################
##########################################################################
torch.cuda.empty_cache()
