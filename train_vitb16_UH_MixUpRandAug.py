...
from utils import vit_experiment_helpers as vit_helper
...
MIXUP_ALPHA = 0.2
dataset_name = "UH"
model_name = "vitb16-MixUpRandAug"
DEST_ROOT = f"/nobackup/users/birdy/{model_name}-{dataset_name}"

dataset_name = "UH"
data_dir = "/home/birdy/meng_thesis/data/split_MGL1704_data"
train_dataset = torchvision.datasets.ImageFolder(f"{data_dir}/train", helper.data_transforms["train_padToSize"])
val_dataset = torchvision.datasets.ImageFolder(f"{data_dir}/test", helper.data_transforms["val_padToSize"])
class_names = train_dataset.classes
NUM_CLASSES = len(train_dataset.classes)


mixup_fn = vit_helper.Mixup(alpha=MIXUP_ALPHA)

train_dataloader = torch.utils.data.DataLoader(
    train_dataset, batch_size=BATCH_SIZE, shuffle=True, 
    num_workers=NUM_WORKERS,
    collate_fn=lambda batch: vit_helper.mixup_collate_fn(batch, mixup_fn, NUM_CLASSES))
